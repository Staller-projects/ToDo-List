        ...
        tasks.forEach(task => {
            taskDisplayHeroDiv.innerHTML += `  
                <div class="relative" id="${task.id}">
                    <input type="text" id="_task${task.id}" value="${task.task}"  class="${task.complete ? 'line-through' : ''} truncate w-full dark:bg-slate-700 bg-slate-300 dark:text-white text-black lg:px-5 lg:py-2 px-3 lg:px-8 px-6 py-2 dark:text-white text-black lg:text-base text-sm font-semibold rounded-lg outline-none" disabled >
